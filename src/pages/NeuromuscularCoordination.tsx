      ...
      <main className="flex-1 pb-12 max-w-4xl mx-auto w-full">
        ...
        <FadeContent blur={true} duration={1000} initialOpacity={0}>
          <div className="px-4 pt-4 pb-8">
            <StarBorder
              as="button"
              color={workoutStarted ? '#10b981' : '#7311d4'}
              speed="4s"
              className="w-full"
              onClick={() => handleStartWorkout('برنامج السرعة واللياقة', 'التوافق العصبي العضلي')}
              disabled={isStarting || workoutStarted}
            >
              <div className="flex items-center justify-center gap-2 font-bold text-lg py-4">
                {isStarting ? (
                  <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                ) : workoutStarted ? (
                  <>
                    <span className="material-symbols-outlined">check_circle</span>
                    <span>تم تسجيل التدريب بنجاح!</span>
                  </>
                ) : (
                  <>
