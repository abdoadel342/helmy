...
const ProtectedRoute = ({ children }: { children: React.ReactNode }) => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#f7f6f8] dark:bg-[#191022] flex flex-col">
      {/* Header Skeleton */}
      <header className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 border-b border-[#7311d4]/10">
      ...
      <div className="flex-1 px-4 py-6">
        <div className="max-w-md mx-auto space-y-8">
          <div className="w-full aspect-[16/9] bg-zinc-800 rounded-2xl animate-pulse"></div>
          <div className="grid grid-cols-3 gap-4">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-24 bg-zinc-800 rounded-xl animate-pulse"></div>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-4">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="h-32 bg-zinc-800 rounded-2xl animate-pulse"></div>
            ))}
          </div>
        </div>
      </div>
      {/* Bottom Nav Skeleton */}
      <nav className="fixed bottom-0 left-0 right-0 border-t border-[#7311d4]/10 px-6 py-2 flex items-center justify-between max-w-md mx-auto">
        {[1, 2, 3, 4].map(i => (
